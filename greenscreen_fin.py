...
def get_new_vids(fps, height, width):
    ''' 
    Create two new videos. There are three ways to obtain fourcc, dependent on your OS.
    @PARAMS:
        fps - fps of original video, passed to new videos
        height/width - dims of the original video, passed to new videos
    @RETURN: three new video files to be edited
    '''
    try:
      fourcc = cv2.cv.CV_FOURCC(*'XVID')
      masked_video = cv2.VideoWriter("masked_vid.avi", fourcc, fps, (width,height))
      morphed_video = cv2.VideoWriter("morphed_vid.avi", fourcc, fps, (width,height))
      final_video = cv2.VideoWriter("final_vid.avi", fourcc, fps, (width,height))
    except:
      try:
...
def avg_background(video, max_frames = 40):
    """
    Finds temporal average of each pixel over duration of video
    @PARAMS: video file name
    @RETURN: image representing temp avg of video (background)
    """
...
def create_mask(avg_bg, frame, thres):
    """
    Computes distance matrix from norm of the avg_bg-frame. Compares distance
    of each pixel to the threshold to determine if its bg or not. Returns mask.
    """
...
def morph_ops(frame, mask):
    ...
    opened_mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, np.ones((7,7),np.uint8))
    # Connect body parts
...
def edit_video(orig_vid, avg_bg, thres, new_bg=None):
    ...
    masked_video_writer, morphed_video_writer, final_video_writer = get_new_vids(fps, height, width)
    ...
    #for i in range(30):
        ...
        if ret:
            # Thresholding: create mask, apply to frame, add to video
            mask = create_mask(avg_bg, frame, thres)
            ...
            # Green Screen
            if new_bg != None:
                scene_change[np.nonzero(morphed_mask)] = frame[np.nonzero(morphed_mask)]
                scene_change[np.nonzero(0==morphed_mask)] = bgframe[np.nonzero(0==morphed_mask)]
                frame = scene_change

            # CCA and Tracking
            morph_masked2 = copy.deepcopy(np.array(morphed_mask, dtype=np.uint8))
            try :
